...
import { setKeyToLocalStore, shouldPersistSession } from '../utilities/local-storage';
...
describe('settings', () => {
  ...
  it('endpoint objects have correct keys (prod, dev, local)', () => {
    ...
    expect(Object.keys(settings.GIST_QUEUE_API_ENDPOINT).sort()).toEqual(endpointKeys.sort());
    expect(Object.keys(settings.GIST_QUEUE_REALTIME_API_ENDPOINT).sort()).toEqual(
      endpointKeys.sort()
    );
    expect(Object.keys(settings.GIST_VIEW_ENDPOINT).sort()).toEqual(endpointKeys.sort());
  });
});
