...
const MESSAGE_PROPERTY_DEFAULTS = {
  isEmbedded: false,
  elementId: "",
  hasRouteRule: false,
  routeRule: "",
...
describe("mapOverlayPositionToElementId", () => {
  ...
  it("defaults to topCenter for invalid input", () => {
    expect(mapOverlayPositionToElementId("invalid")).toBe(
      "x-gist-floating-top",
    );
  });

  it("defaults to topCenter for undefined input", () => {
    expect(mapOverlayPositionToElementId(undefined)).toBe(
      "x-gist-floating-top",
    );
  });
});
...
describe("hasDisplayChanged", () => {
  ...
  it("returns true when modal position changes", () => {
    ...
    expect(
      hasDisplayChanged(msg, { displayType: "modal", modalPosition: "bottom" }),
    ).toBe(true);
  });
  ...
  it("returns true when maxWidth changes", () => {
    ...
    expect(
      hasDisplayChanged(msg, { displayType: "modal", maxWidth: 600 }),
    ).toBe(true);
  });
});
