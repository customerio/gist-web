...
export const wideOverlayPositions: readonly string[] = ["x-gist-top", "x-gist-bottom", "x-gist-floating-top", "x-gist-floating-bottom"];
...
export function fetchMessageByElementId(elementId: string | null | undefined): GistMessage | null {
  ...
  return Gist.currentMessages.find((message: GistMessage) => message.elementId === elementId) ?? null;
}

export function removeMessageByInstanceId(instanceId: string): void {
  Gist.currentMessages = Gist.currentMessages.filter((message: GistMessage) => message.instanceId !== instanceId);
}
...
export function getCurrentDisplayType(message: GistMessage): "modal" | "overlay" | "inline" | "tooltip" {
  if (message.overlay) {
    return "modal";
...
export function hasDisplayChanged(currentMessage: GistMessage, displaySettings: DisplaySettings): boolean {
