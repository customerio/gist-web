...
describe('inbox-message-manager', () => {
  ...
  it('updateInboxMessagesLocalStore stores messages and dispatches messageInboxUpdated event', async () => {
    const messages: InboxMessage[] = [{ messageId: 'm1', queueId: 'q1', topics: ['news'] }];
  ...
  it('updateInboxMessageOpenState sends PATCH request and updates local store', async () => {
    ...
    const stored: InboxMessage[] = [{ messageId: 'm1', queueId: 'q1', opened: false }];
    const updated: InboxMessage[] = [{ messageId: 'm1', queueId: 'q1', opened: true }];
    vi.mocked(getKeyFromLocalStore).mockReturnValueOnce(stored).mockReturnValueOnce(updated);
    ...
    expect(Gist.events.dispatch).toHaveBeenCalledWith('messageInboxUpdated', updated);
  });
  ...
  it('removeInboxMessage logs view, removes from store, dispatches event', async () => {
    ...
    vi.mocked(getKeyFromLocalStore).mockReturnValueOnce(stored).mockReturnValueOnce(afterRemove);
